...
/* context-free grammar conversion group */
...
int main() {
    freopen("input_variant_15.txt", "r", stdin);
...
Grammar read_grammar() {
    ...
    /* read input line by line, finish when encounter an empty line */
    string line;
...
Grammar eliminate_useless_symbols(const Grammar &g) {
    ...
    for (auto &rule : g) {
        ...
        /* if contains nonproductive symbols on it's right side - skip it */
...
Grammar transform_into_cnf(const Grammar &g) {
    ...
    /* define a producer that will give us next available variable */
    ...
    for (auto &rule : g) {
        ...
        /* chain from the back to the forth */
